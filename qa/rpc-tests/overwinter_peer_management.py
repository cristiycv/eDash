...
#
# In this test we connect Sprout and Overwinter mininodes to a edashd node
# which will activate Overwinter at block 10.
#
# We test:
# 1. the mininodes stay connected to edash with Sprout consensus rules
# 2. when Overwinter activates, the Sprout mininodes are dropped
# 3. new Overwinter nodes can connect to edash
# 4. new Sprout nodes cannot connect to edash
#
# This test *does not* verify that prior to Overwinter activation, the edashd
...
class OverwinterPeerManagementTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify mininodes are still connected to edashd node
        ...
        # Mininodes send ping message to edashd node.
        pingCounter = 1
        for node in nodes:
        ...
        # Connect a new Overwinter mininode to the edashd node, which is accepted.
        nodes.append(NodeConn('127.0.0.1', p2p_port(0), self.nodes[0], test, "regtest", True))
        time.sleep(3)
        assert_equal(11, len(self.nodes[0].getpeerinfo()))

        # Try to connect a new Sprout mininode to the edashd node, which is rejected.
