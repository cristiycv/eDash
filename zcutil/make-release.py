...
def main(args=sys.argv[1:]):
    """
    Perform the final edash release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libedashconsensus.pc',
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'edash Company'
    sh_log(
        'debchange',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "edash-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "edash-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './edash-make-release.log'
    ...
    logging.info('edash make-release.py debug log: %r', logname)
