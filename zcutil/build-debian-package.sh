...
PACKAGE_VERSION=$($SRC_PATH/src/edashd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/edashd $DEB_BIN
cp $SRC_PATH/src/edash-cli $DEB_BIN
cp $SRC_PATH/zcutil/fetch-params.sh $DEB_BIN/edash-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/edashd.1 $DEB_MAN
cp $SRC_DOC/man/edash-cli.1 $DEB_MAN
cp $SRC_DOC/man/edash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/edashd.bash-completion $DEB_CMP/edashd
cp $SRC_PATH/contrib/edash-cli.bash-completion $DEB_CMP/edash-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/edashd.1
gzip --best -n $DEB_MAN/edash-cli.1
gzip --best -n $DEB_MAN/edash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/edashd $DEB_BIN/edash-cli
