...
Jack Grigg (22):
      ...
      Instruct users to run edash-fetch-params if network params aren't available
      ...
      Throw an error if edash.conf is missing
      Show a friendly message explaining why edashd needs a edash.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty edash.conf during performance measurements
      Create empty edash.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from edash.conf to avoid problems with command line tools
      Closes #1097 so edash-cli now displays license info like edashd.
