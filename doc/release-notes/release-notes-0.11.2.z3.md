...
Nathan Wilcox (4):
      ...
      Add a edash-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the edash-gtest run; invoke with make edash-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to edash-gtest.
      ...
      Small nit fixes
      Run `edash-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libedash` edash protocol API and crypto constructions surrounding the zkSNARK circuit.
      ...
      Rename edash_ constants to ZC_.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to edash.conf and edashd.pid in qa/ and src/
