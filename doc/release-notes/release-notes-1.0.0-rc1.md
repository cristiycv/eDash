...
Jay Graber (17):
      ...
      Refer to edash wiki in INSTALL
      Rm bitcoin logo
      Rm build-unix.md, to keep single copy of build instructions for edash on github wiki
      Rm Bitcoin-specific documentation
      Add note that document is not updated for edash to translation policy
      Rm doc for disabled REST interface
      Change alpha to beta testnet, add edash hidden service
      Improve documentation on connecting to edash hidden server
      Improve documentation on connecting to edash hidden server
      Update tor.md
...
Kevin Gallagher (24):
      ...
      Adds edash/edash.h to LIBedash sources
      Adds edash/Proof.hpp to LIBedash_H
      Add alertkeys.h to libbitcoin_server_a_SOURCES
      Adds files in src/edash/circuit to libedash_a_SOURCES
      Adds zcbenchmarks.h to libbitcoin_wallet_a_SOURCES
      Adds json_test_vectors.h to edash_gtest_SOURCES
      Adds additional licenses to Debian copyright file
      Updates edash Core developers -> edash developers
