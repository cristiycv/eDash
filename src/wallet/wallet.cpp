...
bool CWallet::AddZKey(const libedash::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libedash::PaymentAddress &address,
                                    const libedash::ReceivingKey &rk,
...
bool CWallet::LoadCryptedZKey(const libedash::PaymentAddress &addr, const libedash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libedash::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libedash::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libedash::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libedash::ViewingKey &vk)
{
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pedashParams, wtxItem.second.joinSplitPubKey);
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pedashParams, tx.joinSplitPubKey);
    ...
    bool ignoreUnspendable)
{
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pedashParams, wtx.joinSplitPubKey);
            try {
