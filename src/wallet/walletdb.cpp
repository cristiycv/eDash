...
bool CWalletDB::WriteCryptedZKey(const libedash::PaymentAddress & addr,
...
bool CWalletDB::WriteZKey(const libedash::PaymentAddress& addr, const libedash::SpendingKey& key, const CKeyMetadata &keyMeta)
{
    nWalletDBUpdated++;
...
bool CWalletDB::WriteViewingKey(const libedash::ViewingKey &vk)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("vkey"), vk), '1');
}

bool CWalletDB::EraseViewingKey(const libedash::ViewingKey &vk)
{
    nWalletDBUpdated++;
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if (strType == "czkey")
        {
            ...
            uint256 rkValue;
            ssValue >> rkValue;
            libedash::ReceivingKey rk(rkValue);
