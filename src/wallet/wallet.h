...
{
public:
    libedash::PaymentAddress address;
    ...
    CNoteData(libedash::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libedash::PaymentAddress a, uint256 n) :
...
struct CNotePlaintextEntry
{
    JSOutPoint jsop;
    libedash::PaymentAddress address;
    libedash::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libedash::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    bool AddZKey(const libedash::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libedash::SpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libedash::PaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libedash::PaymentAddress &addr, const libedash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libedash::PaymentAddress &address, const libedash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);

    //! Adds a viewing key to the store, and saves it to disk.
    bool AddViewingKey(const libedash::ViewingKey &vk);
    bool RemoveViewingKey(const libedash::ViewingKey &vk);
    //! Adds a viewing key to the store, without saving it to disk (used by LoadWallet)
    bool LoadViewingKey(const libedash::ViewingKey &dest);
    ...
    boost::optional<uint256> GetNoteNullifier(
        ...
        const libedash::PaymentAddress& address,
    ...
    void GetFilteredNotes(std::vector<CNotePlaintextEntry>& outEntries,
                          std::set<libedash::PaymentAddress>& filterAddresses,
                          int minDepth=1,
                          bool ignoreSpent=true,
                          bool ignoreUnspendable=true);
