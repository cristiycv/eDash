...
#include "wallet.h"
#include "walletdb.h"
#include "edash/IncrementalMerkleTree.hpp"
...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            auto hSig = prevJoinSplit.h_sig(*pedashParams, tx_.joinSplitPubKey);
            try {
    ...
    uint256 anchor)
{
    ...
    boost::array<libedash::JSInput, ZC_NUM_JS_INPUTS> inputs{info.vjsin[0], info.vjsin[1]};
    boost::array<libedash::JSOutput, ZC_NUM_JS_OUTPUTS> outputs{info.vjsout[0], info.vjsout[1]};
        ...
        &esk); // parameter expects pointer to esk, so pass in address
    {
        ...
        if (!(jsdesc.Verify(*pedashParams, verifier, joinSplitPubKey_))) {
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pedashParams, joinSplitPubKey_);
    ...
    for (int i = 0; i < ZC_NUM_JS_OUTPUTS; i++) {
        ...
        libedash::PaymentAddress zaddr = output.addr; // randomized output
