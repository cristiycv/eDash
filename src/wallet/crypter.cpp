...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libedash::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libedash::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libedash::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libedash::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libedash::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libedash::PaymentAddress &address, libedash::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libedash::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libedash::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
