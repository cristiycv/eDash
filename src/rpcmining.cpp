...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or edash.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "edashd compiled without wallet and -mineraddress not set");
#endif
    }
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "edash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "edash is downloading blocks...");
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    ...
    CAmount nCommunityReward = 0;
    if ((nHeight > 0) && (nHeight <= Params().GetConsensus().GetLastCommunityRewardBlockHeight())) {
        nCommunityReward = nReward/5;
        nReward -= nCommunityReward;
    }
    UniValue result(UniValue::VOBJ);
    result.push_back(Pair("miner", ValueFromAmount(nReward)));
    result.push_back(Pair("founders", ValueFromAmount(nCommunityReward)));
    return result;
}
