...
bool CBasicKeyStore::AddSpendingKey(const libedash::SpendingKey &sk)
{
...
bool CBasicKeyStore::AddViewingKey(const libedash::ViewingKey &vk)
{
    LOCK(cs_SpendingKeyStore);
...
bool CBasicKeyStore::RemoveViewingKey(const libedash::ViewingKey &vk)
{
    LOCK(cs_SpendingKeyStore);
...
bool CBasicKeyStore::HaveViewingKey(const libedash::PaymentAddress &address) const
{
    LOCK(cs_SpendingKeyStore);
    return mapViewingKeys.count(address) > 0;
}

bool CBasicKeyStore::GetViewingKey(const libedash::PaymentAddress &address,
                                   libedash::ViewingKey &vkOut) const
{
    LOCK(cs_SpendingKeyStore);
