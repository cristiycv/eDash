...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "edash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "edashd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8242, 18242));
...
static void ZC_LoadParams()
{
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path))) {
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the edash network parameters in the following directory:\n"
              "%s\n"
              "Please run 'edash-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pedashParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. edash is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. edash is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. edash is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize edash circuit parameters
    ZC_LoadParams();
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of edash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart edash to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("edash was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild edash with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("edash was not built with wallet support. Set -mineraddress, or rebuild edash with wallet support."));
    }
 #endif // !ENABLE_WALLET
